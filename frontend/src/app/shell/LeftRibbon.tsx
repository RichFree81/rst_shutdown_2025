...
export default function LeftRibbon() {
  ...
  const activeHome = useMemo(() => loc.pathname === "/", [loc.pathname]);
  ...
  return (
    <>
      ...
      <aside className="fixed left-0 top-12 z-20 h-[calc(100vh-48px)] w-12 border-r border-border-subtle bg-neutral-200 flex flex-col items-center py-1 gap-1">
        <Link key="home" to="/" title="Home" className={`group rounded-xl p-2 ${activeHome ? "text-brand-navy border-l-2 border-brand-copper-300" : "text-text-secondary hover:text-brand-navy"}`}>
          <svg viewBox="0 0 24 24" className="h-6 w-6" fill="currentColor"><path d="M12 3 3 10h2v9h6v-6h2v6h6v-9h2z"/></svg>
        </Link>
        {domains.map((d) => (
          <Link key={d.id} to={`/d/${d.id}`} title={d.title} className={`group rounded-xl p-2 ${activeDomainId === d.id ? "text-brand-navy border-l-2 border-brand-copper-300" : "text-text-secondary hover:text-brand-navy"}`}>
            {d.icon.render()}
          </Link>
        ))}
        <div className="mt-auto">
          <button title="Settings" className="rounded-xl p-2 text-text-secondary hover:text-brand-navy">
